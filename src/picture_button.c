...
void draw_picture_button(const struct widget_state* w)
{
  SDL_Rect inner_rect;
  ...
  struct picture_button_state* state = w->state.picture_button;
...
enum punk_click_type punk_picture_button(const char* img_path, const struct punk_style* style)
{
  ...
  struct widget_state* w = find_widget(PICTURE_BUTTON, &layout->current_child);
  if (w)
  {
    w->needs_to_be_rendered = 1;

    // If the caption has changed then force a re-draw.
    struct picture_button_state* current_state = w->state.picture_button;
  ...
  else
  {
    ...
    struct picture_button_state* state =
      (struct picture_button_state*)malloc(sizeof(struct picture_button_state));
    strcpy(state->img_path, img_path);
    w->state.picture_button = state;
    w->draw = &draw_picture_button;
  }
